...
FINAL_CSV_NAME = "experiment_2n_2m_all.csv"  # 合并后的总表名

# 2. 拓扑库（已有拓扑，非随机、可复现）
#    当前示例：ny20_deg5（20节点、平均度数约5），可以在这里再加别的拓扑。
TOPOLOGY_LIBRARY: Dict[str, Dict[str, Any]] = {
    "toy2_node": {
        "description": "Two nodes connected by a single bidirectional link",
        "adjacency": [
            [0, 1],
            [1, 0]
        ]
...
GENERATION_CONFIG: Dict[str, Any] = {
    ...
    "topology_names": ["toy2_node"],

    # （2）模型名称（固定，不参与全排列；想换模型改这里）
    "model_name": "ToyModel-2L",
    ...
    "pricing_profiles": [
        {"user_price_per_month": 12.0, "tokens_per_user": 1}
    ],
}
