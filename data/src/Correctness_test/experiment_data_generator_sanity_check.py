...
FINAL_CSV_NAME = "experiment_sanity_check_all.csv"  # 合并后的总表名

# 2. 拓扑库（已有拓扑，非随机、可复现）
#    当前示例：ny20_deg5（20节点、平均度数约5），可以在这里再加别的拓扑。
TOPOLOGY_LIBRARY: Dict[str, Dict[str, Any]] = {
    "toy1_node": {
        "description": "Single node topology for sanity test",
        "adjacency": [
            [0]
        ]
    }
...
GPU_CATALOG: Dict[str, Dict[str, Any]] = {
    "ToyGPU10": {
        "name": "ToyGPU10",
        "G_TFLOPS": 10.0,                 # 算力上限 10 TFLOPs/s
        "VRAM_bytes": int(100 * 1024**3), # 100 GB 显存，保证不会成限制项
        "cost_per_GB_month": 1.0,         # 显存单价 1 $/GB/月
    }
}
...
MODEL_CATALOG: Dict[str, Dict[str, Any]] = {
    "ToyModel-1L": {
        "name": "ToyModel-1L",
        "num_layers": 1,
        "total_params": float(1e6),  # 随便给个不大的参数量
        "bytes_per_param": 2,
        "hidden_size": 1,
        "num_attention_heads": 1,
        "num_key_value_heads": 1,
        "flops_per_token_per_layer": float(1e12),  # 1e12 FLOPs = 1 TFLOP
        "activation_mb_per_boundary": 1.0,         # 这在 K=1 时不会用到
    }
}

# 6. 运行时配置（KV / Activation 计算用），不参与全排列
RUN_CONFIG: Dict[str, Any] = {
    "seq_len": 1,   # KV cache 中的上下文长度（用于显存估算）
    "batch_size": 1,   # 批量大小
...
GENERATION_CONFIG: Dict[str, Any] = {
    ...
    "topology_names": ["toy1_node"],

    # （2）模型名称（固定，不参与全排列；想换模型改这里）
    "model_name": "ToyModel-1L",
    ...
    "pricing_profiles": [
        {"user_price_per_month": 5.0, "tokens_per_user": 1}
    ],
}
