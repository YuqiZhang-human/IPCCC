...
def process_test_case(row):
    ...
    adjacency = test_data.get("adjacency_matrix", [])
    distance = test_data.get("distance_matrix", [])
    bandwidth_matrix = test_data.get("bandwidth_matrix", [])
    resource_demands = test_data.get("resource_demands", [])
    data_sizes = test_data.get("data_sizes", [])
    computation_capacity = test_data.get("computation_capacity", [])
    ...
    degree_max = 0.0
    if adjacency:
        deg_list = [sum(1 for x in row_ if x != 0) for row_ in adjacency]
        deg_arr = np.array(deg_list, dtype=float)
        if deg_arr.size > 0:
            degree_std = float(np.std(deg_arr))
            degree_min = float(np.min(deg_arr))
            degree_max = float(np.max(deg_arr))

    # 2) 最短路径长度统计（剔除0对角线和999不可达）
    avg_shortest_path_len = 0.0
    shortest_path_len_std = 0.0
    shortest_path_len_max = 0.0
    if distance:
        dist_arr = np.array(distance, dtype=float)
        mask = (dist_arr > 0) & (dist_arr < 999)
        valid = dist_arr[mask]
        if valid.size > 0:
            avg_shortest_path_len = float(np.mean(valid))
            shortest_path_len_std = float(np.std(valid))
            shortest_path_len_max = float(np.max(valid))

    # 3) 链路带宽统计（只看有边的地方）
    link_bandwidth_mean = 0.0
    link_bandwidth_min = 0.0
    link_bandwidth_max = 0.0
    if adjacency and bandwidth_matrix:
        bw_arr = np.array(bandwidth_matrix, dtype=float)
        adj_arr = np.array(adjacency, dtype=int)
        mask = adj_arr > 0
        valid_bw = bw_arr[mask]
        if valid_bw.size > 0:
            link_bandwidth_mean = float(np.mean(valid_bw))
            link_bandwidth_min = float(np.min(valid_bw))
            link_bandwidth_max = float(np.max(valid_bw))

    # 4) 模块资源需求统计
    seg_compute_demand_mean = 0.0
    seg_compute_demand_std = 0.0
    seg_memory_demand_mean = 0.0
    seg_memory_demand_std = 0.0
    global_compute_per_user = 0.0
    total_memory_demand = 0.0

    if resource_demands:
        rd_arr = np.array(resource_demands, dtype=float)  # shape = (module_count, 2)
        if rd_arr.size > 0:
            comp_arr = rd_arr[:, 0]
            mem_arr = rd_arr[:, 1]
            seg_compute_demand_mean = float(np.mean(comp_arr))
            seg_compute_demand_std = float(np.std(comp_arr))
            seg_memory_demand_mean = float(np.mean(mem_arr))
            seg_memory_demand_std = float(np.std(mem_arr))
            global_compute_per_user = float(np.sum(comp_arr))   # 所有模块合计单用户算力需求
            total_memory_demand = float(np.sum(mem_arr))        # 单条链上所有模块的总显存需求

    # 5) 模块边界数据量统计（通信负载特征）
    seg_boundary_mb_mean = 0.0
    seg_boundary_mb_std = 0.0
    seg_boundary_mb_max = 0.0
    if data_sizes:
        ds_arr = np.array(data_sizes, dtype=float)
        if ds_arr.size > 0:
            seg_boundary_mb_mean = float(np.mean(ds_arr))
            seg_boundary_mb_std = float(np.std(ds_arr))
            seg_boundary_mb_max = float(np.max(ds_arr))

    # 6) 全局资源容量
    total_compute_capacity = 0.0
    total_memory_capacity = 0.0
    if computation_capacity:
        cap_arr = np.array(computation_capacity, dtype=float)  # shape = (node_count, 2)
        if cap_arr.size > 0:
    ...
    result = {
        ...
        'seg_compute_demand_std': seg_compute_demand_std,
        'seg_memory_demand_mean': seg_memory_demand_mean,
        'seg_memory_demand_std': seg_memory_demand_std,
        'seg_boundary_mb_mean': seg_boundary_mb_mean,
        'seg_boundary_mb_std': seg_boundary_mb_std,
        'seg_boundary_mb_max': seg_boundary_mb_max,
    ...
    try:
        ...
        try:
            ...
            if multi_func_result:
                ...
                if max_profit_plan is not None:
                    ...
                    mf_profit_chain_count = max_profit_plan[7]

                    if mf_profit_chain_count > 0:
                        result['multi_func_profit_avg_users_per_chain'] = float(
                            mf_profit_total_users) / mf_profit_chain_count
                        result['multi_func_profit_avg_nodes_per_chain'] = float(
                            mf_profit_used_nodes) / mf_profit_chain_count
                    else:
                        result['multi_func_profit_avg_users_per_chain'] = 0.0
                        result['multi_func_profit_avg_nodes_per_chain'] = 0.0

                    # 全局算力利用率： (总算力需求 / 总算力容量)
                    if total_compute_capacity > 0 and global_compute_per_user > 0:
                        result['multi_func_profit_compute_util'] = (
                            global_compute_per_user * mf_profit_total_users
                ...
                if max_users_plan is not None:
                    ...
                    mf_users_total_users = max_users_plan[4]
                    mf_users_used_nodes = max_users_plan[5]
                    mf_users_chain_count = max_users_plan[7]

                    if mf_users_chain_count > 0:
                        result['multi_func_max_users_avg_users_per_chain'] = float(
                            mf_users_total_users) / mf_users_chain_count
                        result['multi_func_max_users_avg_nodes_per_chain'] = float(
                            mf_users_used_nodes) / mf_users_chain_count
                    else:
                        result['multi_func_max_users_avg_users_per_chain'] = 0.0
                        result['multi_func_max_users_avg_nodes_per_chain'] = 0.0

                    # 全局算力利用率
                    if total_compute_capacity > 0 and global_compute_per_user > 0:
                        result['multi_func_max_users_compute_util'] = (
                            global_compute_per_user * mf_users_total_users
                        ) / total_compute_capacity
                    else:
        ...
        try:
            ...
            if sp_result is not None:
                ...
                sp_total_users = sp_result["total_users"]
                sp_used_nodes = sp_result["used_nodes"]
                sp_chain_count = sp_result["chain_count"]

                if sp_chain_count > 0:
                    result['sp_avg_users_per_chain'] = float(sp_total_users) / sp_chain_count
                    result['sp_avg_nodes_per_chain'] = float(sp_used_nodes) / sp_chain_count
                else:
                    result['sp_avg_users_per_chain'] = 0.0
                    result['sp_avg_nodes_per_chain'] = 0.0

                if total_compute_capacity > 0 and global_compute_per_user > 0:
                    result['sp_compute_util'] = (
                        global_compute_per_user * sp_total_users
                    ) / total_compute_capacity
                else:
        ...
        try:
            ...
            if single_func_result:
                ...
                sf_total_users = single_func_result[4]
                sf_used_nodes = single_func_result[5]
                sf_chain_count = single_func_result[7]

                if sf_chain_count > 0:
                    result['single_func_avg_users_per_chain'] = float(sf_total_users) / sf_chain_count
                    result['single_func_avg_nodes_per_chain'] = float(sf_used_nodes) / sf_chain_count
                else:
                    result['single_func_avg_users_per_chain'] = 0.0
                    result['single_func_avg_nodes_per_chain'] = 0.0

                if total_compute_capacity > 0 and global_compute_per_user > 0:
                    result['single_func_compute_util'] = (
                        global_compute_per_user * sf_total_users
                    ) / total_compute_capacity
                else:
