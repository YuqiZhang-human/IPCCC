...
本版本显存模型与 multi/single 优化器保持一致：
- resource_demands[][1] 只表示 KV 显存/用户；
- 额外的 weight_memory_demands[] 表示权重显存（一次加载，多链复用）；
- 节点显存约束：KV*用户数 + 新增权重 ≤ 剩余显存；
- 成本中：KV 显存按用户数线性计价，权重显存按一次性静态成本计价。
"""
...
class ShortestPathOptimizer:
    def __init__(self, test_case: Dict[str, Any]):
        ...
        - "weight_memory_demands": List[weight_mem_GB], 长度 M（可选，无则视为 0）
        ...
        # 每个模块的 per-user 需求（第二项只表示 KV per user）
        ...
        # 当前策略下：每个节点已加载过哪些模块的权重（多链之间权重复用）
    ...
    def shortest_path_deployment(self) -> Optional[Dict[str, Any]]:
        ...
        返回一个 dict，包含：
            ...
            total_profit, total_users, used_nodes,
        ...
        while True:
            ...
            # 注意：per_user_dep 已经包含了“权重显存成本 / max_users”的摊销，
            # 因此乘回 chain_users 后，会变成：
            #   KV*users + compute*users + static_weight_cost
    ...
    def _find_best_feasible_chain(self) -> Optional[PathResult]:
        ...
        # dp[layer][node] = 走到第 layer 个模块部署在 node 上的最小单人成本（不含静态权重成本）
        ...
        for node in range(N):
            ...
            # 判断新链在该节点部署模块0时，是否至少能放下 1 个用户：
            #   comp_demand_0 <= comp_cap
            #   kv_demand_0 + new_weight_0 <= mem_cap
            ...
            # DP 内部的 per-user 成本只算「算力 + KV 显存」，不算静态权重成本
        ...
        for last_node in range(N):
            ...
            # 计算本链的静态权重显存成本（只对尚未加载过的模块权重计价一次）
            static_weight_cost = self._weight_cost_for_deployment_static(deployment)

            # 为了让 chain_deploy_cost = per_user_dep * users
            # 等价于「(KV+compute)*users + static_weight_cost」，这里将静态成本均摊到每个用户
            dep_cost_per_user = dep_cost_per_user_excl_weight + (
                static_weight_cost / float(max_users)
            )
    ...
    def _deploy_cost_per_user(self, module_idx: int, node_idx: int) -> float:
        """
        单用户在 node_idx 上部署 module_idx 的成本（不含静态权重成本）：
        per-user 部署成本 = compute_demand * gpu_cost + kv_mem_demand * mem_cost
        """
    ...
    def _weight_cost_for_deployment_static(self, deployment: List[int]) -> float:
        """
        计算本次链部署带来的「静态权重显存成本」：
        - 仅对尚未在节点上加载过的 (module, node) 组合计一次，
          之后的链在同节点复用该权重不再重复计成本。
        """
        static_cost = 0.0
        for m_idx, node in enumerate(deployment):
            weight_mem = self.weight_memory_demands[m_idx]
