...
"""
单功能部署优化器（单链节点不重复 + 多链可复用版）

特性：
- 单条链内部：每个模块必须部署在不同节点上（single-function in-chain）
- 多条链之间：节点可以被多条链复用，只要剩余算力 / 显存与链路带宽足够
- 资源建模：
...
- 多链部署流程：
    * 从初始资源开始（所有节点按其初始算力 / 显存可用）
    * 反复：
        1. 在当前“剩余资源”下求一条单链的最优方案（最大利润）
        2. 计算该链的最大支持用户数 user_count
        3. 累计成本 / 利润 / 用户数等指标
        4. 从剩余资源中扣除该链的资源占用（节点仍可在后续链中复用，直到资源耗尽）
...
class SingleFunctionOptimizer:
    ...
    def __init__(self, test_data: Dict[str, Any]) -> None:
        ...
        # 节点可用状态：
        # True 表示该节点当前仍有正的算力或显存剩余（可以在新的链中继续部署模块）
        # False 表示该节点算力和显存都耗尽，无法再部署新的模块
    ...
    def find_best_single_chain(
        ...
        """
        在当前剩余资源状态下，枚举所有单条链部署方案，
        返回最优方案（以利润最大为目标）。

        约束：
        - 同一条链中，每个模块必须部署在不同节点上（单链节点不重复）
        - 不限制该节点之前是否被其他链使用过，只看当前剩余资源是否允许
        """
        n = self.node_count
        m = self.module_count
        ...
        # 如果当前仍有正资源的节点数 < 模块数，则不可能找到单链节点不重复的方案
        ...
        # 回溯搜索：为每个模块选择一个“当前仍有资源且在本链未用过”的节点
        used_nodes = [False] * n
        deployment = [0] * m
